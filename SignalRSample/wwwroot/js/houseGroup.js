...
connectionHouse.on("triggerHouseNotification", (houseName) => {
    toastr.success(`A new notification for ${houseName} has been lunched.`)
});

connectionHouse.on("newMemeberAddedToHouse", (houseName) => {
    toastr.success(`Member has subscribed to ${houseName}`)
});
connectionHouse.on("memeberRemovedFromHouse", (houseName) => {
    toastr.warning(`Member has unSubscribed from ${houseName}`)
});

connectionHouse.on("subscriptionStatus", (srtGroupJoined, houseName, haSubscribed) => {
    lbl_houseJoined.innerText = srtGroupJoined;
    if (haSubscribed) {
        switch (houseName) {
            case 'slytherin':
